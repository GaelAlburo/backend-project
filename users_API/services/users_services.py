...
class UserService:
    ...
    def get_all_users(self):
        try:
            ...
            return jsonify({"error": f"Error fetching all users from database: {e}"}), 500
        

    def get_users_by_user_email(self, user_email):

        try:
            user = self.db_conn.db.users.find_one({"email": user_email})    #filter by user email
            return user
        except Exception as e:
            self.logger.error(f"Error fetching user by email from database: {e}")
            return jsonify({"error": f"Error fetching user by email from database: {e}"}), 500
        
    def get_users_by_user_id(self, user_id):

        try:
            user = self.db_conn.db.users.find_one({"_id": user_id})    #filter by user email
            return user
        except Exception as e:
            self.logger.error(f"Error fetching user by id from database: {e}")
            return jsonify({"error": f"Error fetching user by id from database: {e}"}), 500
    ...
    def create_new_user(self, new_user):
        ...
        try:
            ...
            if existing_user:
                self.logger.error(f"Error adding new user to database: Email is already registered")
                return "Email is already registered", 500

            max_id = self.db_conn.db.users.find_one(sort=[("_id", -1)])["_id"]      # Gets the highest id
            next_id = max_id + 1
